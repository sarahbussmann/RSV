...
n = 15            # the number of attributes (columns)
RSV_X <- matrix(0,nrow=m,ncol=n)   # creates a matrix of zeros that is the size of mxn
colnames(RSV_X) <- c("Age_25","CS1_25","R1_25","WEL1_25","INSLT1_25","Age_50","CS1_50","R1_50","WEL1_50","INSLT1_50","Age_75","CS1_75","R1_75","WEL1_75",
                     "INSLT1_75")
...
quantileINSLT <- quantile(Ymatrix$INSLT, probs =seq(0,1,0.25))
...
# write.csv(RSV_X, file="C:/Users/Carley/Documents/csvFiles/RSV_X_ThreeQuantiles_NineRealAttributes_TwoCond_Diff.csv", row.names=FALSE)

WELfinal <- subset(RunSubset, time == 45, select = c(WEL))   # takes WEL for each student's final assessment
Sorted_reals <- subset(SortByTime, select = c(CS2, R2, WEL2, INSLT2, CS3, R3, WEL3, INSLT3, CS4, R4, WEL4, INSLT4))
...
# Difference <- attributes2$WEL - attributes1$WEL

Difference <- as.data.frame(Difference)

write.csv(Difference, file="difference_ThreeQuantiles_New17RealAttributes_tx37.csv", row.names=FALSE)
write.csv(attributes, file="attributes_ThreeQuantiles_New17RealAttributes_tx37.csv", row.names=FALSE)
